...
$newDateTime = "20/06/2026 | 21:00";
$eventTitle = "הפועל תל אביב נגד מכבי נתניה";
...
function payload_is_target_event($payload, $homeTeam, $awayTeam, $eventTitle, $eventId) {
    if (!is_array($payload)) {
        return false;
    }
    ...
    if ($payloadId === $eventId && $teams === $eventTitle) {
        return true;
    }
...
try {
    ...
    if (table_exists($conn, "events_catalog")) {
        ...
        $likeHome = "%" . $homeTeam . "%";
        $likeAway = "%" . $awayTeam . "%";
        $select->bind_param("ssss", $eventTitle, $eventId, $likeHome, $likeAway);
        $select->execute();
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (!payload_is_target_event($payload, $homeTeam, $awayTeam, $eventTitle, $eventId)) {
                continue;
            }
    ...
    if (table_exists($conn, "orders")) {
        ...
        $likeHome = "%" . $homeTeam . "%";
        $likeAway = "%" . $awayTeam . "%";
        $updateOrders->bind_param("sssss", $newDateTime, $eventTitle, $eventId, $likeHome, $likeAway);
    ...
    echo "<h2>עדכון תאריך משחק לצורך בדיקת זיכוי חלקי</h2>";

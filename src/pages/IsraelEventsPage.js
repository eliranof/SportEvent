...
import { getIsraelSections } from "../services/eventsService";
import "./IsraelEventsPage.css";
import bg from "../assets/img/incoming-games.jpg";

export default function IsraelEventsPage() {
  const navigate = useNavigate();
  const sections = getIsraelSections();
  ...
  const handleOpenEvent = (eventItem) => {
    navigate(`/event/${eventItem.id}`, {
      state: {
        selectedEvent: {
          ...eventItem,
          category: eventItem.category || eventItem.tag || "אירוע בארץ",
        },
      },
    });
  };
  ...
  return (
    ...
    >
      ...
      <header className="events-page__topbar">
        ...
        <h1 className="events-page__title">אירועים בארץ</h1>
